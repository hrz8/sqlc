...
func (m *GoCode) CloneVT() *GoCode {
	...
	r := &GoCode{
		...
		EmitWithExtraQuery:        m.EmitWithExtraQuery,
	}
...
func (this *GoCode) EqualVT(that *GoCode) bool {
	...
	if this.EmitWithExtraQuery != that.EmitWithExtraQuery {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *GoCode) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 28:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field EmitWithExtraQuery", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
